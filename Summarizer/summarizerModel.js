(function () {
    var Summarizer = function () {
        ...
        this.isEditingNumSentences=ko.observable(false);
        this.summarizePerParagraphs = function (title, content, callback, sentences_dict) {
        ...
        this.summarizeInOneSentence = function (title, content, callback, sentences_dict) {
            var summary = [], err = false;
            if (arguments.length < 3) {
                if (content.constructor === Function) {
                    callback = content;
                    content = title;
            ...
            getSentencesRanks(content, (dict) => {
                getBestSentence(content,dict,(sentence)=>{
        ...
        this.summarizeNSentences = function (title, content,targetSentences, callback, sentences_dict) {
            var summary = [], paragraphs = [], sentence = '', err = false;
            if (arguments.length < 3) {
                if (content.constructor === Function) {
                    callback = content;
            ...
            getSentencesRanks(content, (dict) => {
                ...
                var summaryArr=[];
                splitContentToSentences(content,(sentences)=>{
                    items.forEach((mostValuable)=>{
                        sentences.forEach((sentence,i)=>{
                            formatSentence(sentence, function(formated) {
                                if(mostValuable[0]===formated){
                                    summaryArr.push([i,sentence]);
                                }
                            });
                        });
                ...
                let summary = summaryArr.sort((first,second)=>{
        ...
        this.getSortedSentences = function (content, n, callback, sentences_dict) {
        ...
        var getSentencesRanks = (content, callback, sentences_dict) => {
        ...
        var splitContentToSentences = (content, callback) => {
        ...
        var sentencesIntersection = (sent1, sent2, callback) => {
            let s1 = sent1.split(' ')
            let s2 = sent2.split(' ')
        ...
        var formatSentence = (sentence, callback) => {
        ...
        var getBestSentence = (paragraph, sentences_dict, callback) => {
        ...
        this.mode.subscribe((newVal)=>{
            if(newVal==="mostValuableNSentance"){
                self.isEditingNumSentences(true);
            }else{
                self.isEditingNumSentences(false);
            }
        })
    ...
    ko.applyBindings(new Summarizer(), document.getElementsByTagName('body')[0]);
}())